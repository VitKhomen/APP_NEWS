...
class Payment(models.Model):
    ...
    def is_successful(self):
        '''Перевіряє вдався платеж'''
        return self.status == 'succeeded'

    @property
    def is_pending(self):
        '''Перевіряє що платеж чекає обробки'''
        return self.status in ['pending', 'processing']

    @property
    def can_be_refunded(self):
        '''Перевіряє чи можна вернуть платеж'''
        return self.status == 'succeeded' and self.payment_method == 'stripe'

    def mark_as_succeeded(self):
        '''Відмічає платеж як вдалий'''
...
class Refund(models.Model):
    ...
    @property
    def is_partial(self):
        '''Перевіряє чи поверння є частковим'''
        return self.amount < self.payment.amount

    def process_refund(self):
        '''Обробляеєм повернення'''
        from django.utils import timezone
        self.status = 'succeeded'
        self.processed_at = timezone.now()
        self.save()
...
class WebhookEvent(models.Model):
    '''Випадки webhook від платежних систем'''
    PROVIDER_CHOICES = [
        ('stripe', 'Stripe'),
        ('paypal', 'PayPal'),
    ]
