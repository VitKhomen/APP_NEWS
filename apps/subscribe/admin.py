...
class SubscriptionAdmin(admin.ModelAdmin):
    ...
    def user_link(self, obj):
        """Ссилка на користувача"""
        url = reverse('admin:accounts_user_change', args=[obj.user.pk])
        return format_html('<a href="{}">{}</a>', url, obj.user.username)
    user_link.short_description = 'User'

    def is_active_display(self, obj):
        """Відбраження активності підписки"""
        if obj.is_active:
    ...
    def days_remaining_display(self, obj):
        """Відображення Залишившихся днів"""
    ...
    actions = ['activate_subscriptions',
               'cancel_subscriptions', 'expire_subscriptions']

    def activate_subscriptions(self, request, queryset):
        """Активує вибрані підписки"""
        count = 0
    ...
    def expire_subscriptions(self, request, queryset):
        """Відмічає підписки як закінченні"""
        count = 0
        for subscription in queryset:
            if subscription.status == 'active':
                subscription.expire()
...
class PinnedPostAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request):
        """Забороняєм створення через адмінку"""
        return False
...
class SubscriptionHistoryAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('subscription', 'action',
                       'description', 'metadata', 'created_at')

    def subscription_link(self, obj):
        """Ссилка на підписку"""
        url = reverse('admin:subscribe_subscription_change',
...
# Додаткові налаштування адмінки
