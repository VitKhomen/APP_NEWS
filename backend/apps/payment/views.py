...
def create_checkout_session(request):
    '''Создает Stripe Checkout сессію для оплати підписки'''
    ...
    if serializer.is_valid():
        try:
            with transaction.atomic():
                ...
                if session_data:
                    response_serializer = StripeCheckoutSessionSerializer(
                        session_data)
...
def payment_status(request, payment_id):
    ...
    try:
        ...
        # якшо є session_id перевіряємо статус в stripe
        if payment.stripe_session_id and payment.status in ['pending', 'processing']:
            session_info = StripeService.retrieve_session(
        ...
        response_data = {
            ...
            'message': f'Payment in {payment.status}',
...
class RefundDetailView(generics.RetrieveAPIView):
    '''Детальна інформація о поверненні'''
    serializer_class = RefundSerializer
    permission_classes = [permissions.IsAdminUser]
...
def create_refund(request, payment_id):
    '''Створює повернення платежу'''
    try:
        payment = get_object_or_404(Payment, id=payment_id)

        if not payment.can_be_refunded:
            return Response({
                'error': 'This payment cannot be refundet'
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        if serializer.is_valid():
            with transaction.atomic():
                ...
                if success:
                    ...
                    # якшо це повне вертання відміняємо підписку
...
def payment_analytics(request):
    '''Аналітіка по платежам для адміністратора'''
...
def retry_payment(request, payment_id):
    ...
    try:
        ...
        # створюємо нову сесію для повторной оплати
