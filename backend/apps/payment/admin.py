...
class PaymentAttemptInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = ('stripe_charge_id', 'status',
...
class RefundInline(admin.TabularInline):
    model = Refund
    extra = 0
    readonly_fields = ('amount', 'status', 'stripe_refund_id',
...
class PaymentAdmin(admin.ModelAdmin):
    ...
    def mark_as_succeeded(self, request, queryset):
        ...
        count = queryset.filter(
            status__in=['pending', 'processing']).update(status='succeeded')
    ...
    def export_payments(self, request, queryset):
        ...
        self.message_user(
            request, f'Export functionality will be implemented soon.')
...
class PaymentAttemptAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('payment', 'stripe_charge_id', 'status',
                       'error_message', 'metadata', 'created_at')
...
class WebhookEventAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('provider', 'event_id', 'event_type',
