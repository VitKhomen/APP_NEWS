...
def payment_pre_save(sender, instance, **kwargs):
    '''Обробник перед збереженням платежу'''
    # Зберігаємо минулий статус для відстеження
    if instance.pk:
        try:
...
def payment_post_save(sender, instance, created, **kwargs):
    '''Обробник збереженням платежу'''
    if not created and hasattr(instance, '_previous_status'):
        # Якшо статус змінився на succeeded
        ...
        # Якшо статус змінився на failed
